...
const destinationImages: Record<string, string> = {
  ...
  'vaishno-devi': 'https://images.unsplash.com/photo-1624806992066-5ffcf7ca186b?q=80&w=1200&auto=format&fit=crop',
  ...
  'car': 'https://images.unsplash.com/photo-1549399542-7e3f8b79c341?q=80&w=1200&auto=format&fit=crop',
};

function getCuratedImage(label: string = '', slot: string = '', src?: string): string {
  if (src && !src.startsWith('/images/')) return src;

  const searchKey = `${label} ${slot}`.toLowerCase();
...
export default function PhotoPlaceholder({
  ...
  return (
    <div className={`relative w-full h-full overflow-hidden bg-neutral-900 ${aspectClasses[aspectRatio]} ${className}`}>
      {!imageError ? (
        <img
          src={imageUrl}
          alt={label}
